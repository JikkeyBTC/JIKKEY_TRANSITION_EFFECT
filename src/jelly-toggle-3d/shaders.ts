// Derived from WICG/html-in-canvas Examples/webgpu-jelly-slider/src/index.ts
...
const LINE_RADIUS = MATERIAL.lineRadius;
const LINE_HALF_THICK = MATERIAL.halfThickness;
const JELLY_IOR = MATERIAL.ior;
const JELLY_SCATTER_STRENGTH = MATERIAL.scatter;
const AMBIENT_COLOR = d.vec3f(MATERIAL.ambientColor);
const AMBIENT_INTENSITY = MATERIAL.ambientIntensity;
const AO_STEPS = MATERIAL.aoSteps;
const AO_RADIUS = MATERIAL.aoRadius;
const AO_INTENSITY = MATERIAL.aoIntensity;
const AO_BIAS = MATERIAL.aoBias;
const SPECULAR_POWER = MATERIAL.specularPower;
...
export function createJellyShaders(
  ...
  const lightUniform = root.createUniform(DirectionalLight, {
    direction: std.normalize(d.vec3f(
      MATERIAL.lightDirection[0],
      MATERIAL.lightDirection[1],
      MATERIAL.lightDirection[2],
    )),
    color: d.vec3f(1, 1, 1),
  });
  ...
  const jellyColorUniform = root.createUniform(
    d.vec4f,
    d.vec4f(
      MATERIAL.jellyColor[0],
      MATERIAL.jellyColor[1],
      MATERIAL.jellyColor[2],
...
const fragmentMain = tgpu.fragmentFn({
  ...
  const productionRayMarchPipeline = mode === 'production'
  ...
  const shaders = {
    drawRaymarch(
      ...
      if (diagnosticRayMarchPipeline) {
        if (!diagnostics) throw new Error('Diagnostic attachments are unavailable');
    ...
    destroy(): void {
      if (destroyed) return;
      destroyed = true;
      for (const buffer of ownedUniforms) {
        buffer.destroy();
        accounting.bufferDestroyed();
      }
    },
